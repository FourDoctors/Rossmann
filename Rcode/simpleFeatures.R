...
trts$CompetitionDistance[is.na(trts$CompetitionDistance)] <- Inf
trts$CompetitionDuation[is.na(trts$CompetitionDuration)] <- -Inf
...
trts <- merge(trts, weeklyMeansScaledByStore, by = c("Store", "Week"))
preds <- c(preds, "ScaledWeeklyMean")
monthlyMeansScaledByStore <- ddply(trts, .(Store, Month), summarise,
                                 ScaledMonthlyMean = (mean(LogSales, na.rm=TRUE) -
                                         storeMeans$mean[storeMeans$Store == unique(Store)]
                                 )/storeMeans$sd[storeMeans$Store == unique(Store)])
trts <- merge(trts, monthlyMeansScaledByStore, by = c("Store", "Month"))
preds <- c(preds, "ScaledMonthlyMean")

##going on with scaled means, lets also have a scaled mean for each store

scaledMeanLogSales <- ddply(trts, .(Store), summarise,
                            ScaledStoreMean = (mean(LogSales, na.rm=TRUE) -
                                               mean(trts$LogSales, na.rm=TRUE)) / sd(trts$LogSales, na.rm=TRUE)
                            )
trts <- merge(trts, scaledMeanLogSales, by="Store")
...
trts <- merge(trts, mmsw[, c("Store", "clucut")], by="Store")
...
periodInMonth <- with(trts, 1*(DayOfMonth < 11) +
                            2*(DayOfMonth >= 11 & DayOfMonth < 25) +
                            3 *(DayOfMonth >= 25))
trts$PeriodInMonth <- periodInMonth
...
trts <- merge(trts, monthMeansWideByMonth[, c("Month", "clucutMonths")], by="Month")
...
trts <- merge(trts, weekMeansWideByWeek[, c("Week", "clucutWeeks")], by="Week")
...
trts <- merge(trts, weeklyCustWideByWeek[, c("Week", "clucutWeekByCust")], by = "Week")
